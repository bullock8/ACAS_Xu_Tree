...
# Decision tree stuff
def tree_to_code(tree, feature_names, depth_pad):
    tree_ = tree.tree_
    feature_name = [
...
def get_single_tree(rho_range = [0, 60760], theta_range = [-np.pi, np.pi], psi_range = [-np.pi, np.pi], v_own_range = [100, 1200], v_int_range = [0, 1200]):
    ...
    psi_min = psi_range[0]
    psi_max = psi_range[1]
    v_own_min = v_own_range[0]
    v_own_max = v_own_range[1]
    v_int_min = v_int_range[0]
    v_int_max = v_int_range[1]

    rho_range = np.linspace(rho_min, rho_max, num_rhos)
    theta_range = np.linspace(theta_min, theta_max, num_thetas) #np.linspace(-np.pi, np.pi, num_thetas)
    psi_range = np.linspace(psi_min, psi_max, num_psis)#np.linspace(-np.pi, np.pi, num_psis)
    v_own_range = np.linspace(v_own_min, v_own_max, num_vOwns)
    v_int_range = np.linspace(v_int_min, v_int_max, num_vInts)
    ...
    # index for the command_nums and stored_states data
    index = 0
            ...
            for psi_ind in range(0, num_psis):
                for v_own_ind in range(0, num_vOwns):
                    for v_int_ind in range(0, num_vInts):
                        ...
                        rho = rho_range[rho_ind] #np.random.rand([test_pts,]) * 60760
                        theta = theta_range[theta_ind]
                        psi = psi_range[psi_ind]
                        v_own = v_own_range[v_own_ind]#100 + (1200 - 100) * np.random.rand([test_pts,])
                        v_int = v_int_range[v_int_ind]#0 + (1200 - 0) * np.random.rand([test_pts,])
    ...
    #Print out the tree (TODO)
    print('if ') #print if statement to make sure things stay in bounds
...
if __name__ == "__main__":
    ...
    theta_range = np.linspace(-np.pi, np.pi, num_thetas) #np.linspace(-np.pi, np.pi, num_thetas)
    psi_range = np.linspace(-np.pi, np.pi, num_psis)#np.linspace(-np.pi, np.pi, num_psis)
    ...
    theta_rands = np.random.rand([test_pts,])
    psi_rands = np.random.rand([test_pts,])
    
    for theta_ind in range(0, num_thetas - 1):
        for psi_ind in range(0, num_psis - 1):
            for rho_ind in range(0, num_rhos):
                for v_own_ind in range(0, num_vOwns):
                    for v_int_ind in range(0, num_vInts):
                        theta_min = theta_range[theta_ind]
                        theta_max = theta_range[theta_ind + 1]
                        psi_min = psi_range[psi_ind]
                        psi_max = psi_range[psi_ind + 1]
                        
                        rho = rho_range[rho_ind] #np.random.rand([test_pts,]) * 60760
                        theta = theta_min + (theta_max - theta_min) * np.random.rand([test_pts])
                        psi = psi_min + (psi_max - psi_min) * np.random.rand([test_pts])
                        v_own = v_own_range[v_own_ind]#100 + (1200 - 100) * np.random.rand([test_pts,])
                        v_int = v_int_range[v_int_ind]#0 + (1200 - 0) * np.random.rand([test_pts,])
    ...
    if not generate_figure:
        pickle.dump(stored_states, open('small_train_x.pickle', 'wb'))
        pickle.dump(command_nums, open('small_train_y.pickle', 'wb'))
        pickle.dump(test_states, open('small_test_x.pickle', 'wb'))
        pickle.dump(test_cmds, open('small_test_y.pickle', 'wb'))
    else:
        pickle.dump(stored_states, open('small_train_x_fig.pickle', 'wb'))
        pickle.dump(command_nums, open('small_train_y_fig.pickle', 'wb'))
        pickle.dump(test_cmds, open('small_train_y_tree.pickle', 'wb'))
    ...
    get_single_tree(theta_range=[-np.pi/36, np.pi/36], psi_range=[-np.pi/36, np.pi/36])
